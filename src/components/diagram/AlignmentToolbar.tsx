  ...
  const align = useCallback(
    (type: string) => {
      ...
      const updated = all.map((node) => {
        ...
        const maxX = Math.max(
          ...sel.map((n) => n.position.x + (n.width ?? 150)),
        );
        const minY = Math.min(...sel.map((n) => n.position.y));
        const maxY = Math.max(
          ...sel.map((n) => n.position.y + (n.height ?? 50)),
        );
        ...
        switch (type) {
          ...
          case "distV": {
            ...
            const spanY =
              ...
              (sorted[sorted.length - 1].height ?? 50) -
              sorted[0].position.y;
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              onClick={() => groupNodes(selectedNodes.map((n) => n.id))}
              className="h-8 w-8 rounded-lg hover:bg-accent"
            >
          ...
          <TooltipContent side="top">
            <p className="text-xs font-medium">Group (Ctrl+G)</p>
          </TooltipContent>
        </Tooltip>
      </div>
    </TooltipProvider>
